...
struct PrayerTimeTable {
  ...
  static func forDate(method: PrayerTimeMethod, city: PrayerTimeCity, date: Date) -> PrayerTimeTable {
    let values = PrayerTime.getPrayersForDate(method: method, city: city, date: date)
    return PrayerTimeTable.init(method: method, city: city, date: date, data: values)
  }
  ...
  init(method: PrayerTimeMethod, city: PrayerTimeCity, date: Date, data: [PrayerTime]) {
    self.method = method
    self.city = city
    self.date = date
    self.data = data
