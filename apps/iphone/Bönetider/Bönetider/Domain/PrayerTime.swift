...
struct PrayerTime: Identifiable, Equatable {
  ...
  static func getPrayersForDate(city: PrayerTimeCity, date: Date = Date()) -> [PrayerTime] {
    let values = PrayerTimeData.current.forDate(date)
    return [
      PrayerTime(city: city, type: .fajr, time: values[0]),
      PrayerTime(city: city, type: .shuruk, time: values[1]),
      PrayerTime(city: city, type: .dhohr, time: values[2]),
      PrayerTime(city: city, type: .asr, time: values[3]),
      PrayerTime(city: city, type: .maghrib, time: values[5]),
      PrayerTime(city: city, type: .isha, time: values[6]),
    ]
  }
  ...
  static func getNextPrayers(city: PrayerTimeCity, count: Int, date: Date = Date()) -> [PrayerTime] {
    var result: [PrayerTime] = []
    var current = date
    while result.count < count {
      result.append(contentsOf: getPrayersForDate(city: city, date: current).filter { $0.time > date })
  ...
  var id: String { "\(city.rawValue)|\(type)|\(ISO8601DateFormatter().string(from: time))" }
  ...
  init(city: PrayerTimeCity, type: PrayerTimeType, time: Date) {
    self.city = city
    self.type = type
    self.time = time
  }
  ...
  init(id: String) {
    ...
    guard let cityRawValue = PrayerTimeCity(rawValue: String(parts[0])),
          let typeRawValue = PrayerTimeType(rawValue: String(parts[1])),
          let time = ISO8601DateFormatter().date(from: String(parts[2])) else {
      fatalError("Invalid identifier format: \(id)")
    }
